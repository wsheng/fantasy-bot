...
A fully automated Yahoo Fantasy Basketball daily lineup optimizer. Each morning at 2 AM it fetches your roster, checks the NBA schedule, optimises your active lineup using 30-day player rankings, scans the waiver wire for upgrades, checks IL moves, and emails you a styled HTML report.
...
This fetches your roster, runs the optimizer, scans waivers, and emails the report.
...
# Test optimizer logic with fake data
python /Users/will/Code/fantasy-bot/optimizer.py

# Test waiver scanner with fake data
...
| File | Purpose |
|---|---|
| `main.py` | Daily entry point — orchestrates all modules and sends report |
...
| `nba_schedule.py` | Fetches today's NBA games; checks if a player's team is playing |
| `optimizer.py` | Greedy bipartite lineup builder; bench shape analyser |
| `waiver_scanner.py` | Finds FA upgrades for active lineup and bench |
| `il_manager.py` | Flags players who should move to/from IL |
| `emailer.py` | Builds styled HTML email and sends via Gmail SMTP |
| `requirements.txt` | Python package dependencies |
| `.env.example` | Template for environment variables |
| `oauth2.json.example` | Template for Yahoo OAuth2 token file |
| `untouchables.json` | Auto-generated by weekly.py; read by main.py |
...
`stats.nba.com` occasionally throttles requests or changes its API. The bot automatically falls back to the `balldontlie` public API. If both fail, `has_game_today` will default to `False` for all players (conservative — no one will be benched incorrectly, but you may see active players without games flagged as alerts).
